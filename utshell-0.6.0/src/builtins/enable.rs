...
use crate::findcmd::find_in_path;
use crate::general::{absolute_program, printable_filename};
use crate::pcomplete::{it_builtins, it_disabled, it_enabled, set_itemlist_dirty};
use crate::src_common::*;
use crate::variables::get_string_value;

use crate::builtins::{
    bashgetopt::{internal_getopt, reset_internal_getopt},
    builtins::{num_shell_builtins, shell_builtins, static_shell_builtins},
    cmd::{insert_empty_cmd, set_cmd_enable},
    common::{
        builtin_address_internal, builtin_usage, initialize_shell_builtins, sh_notbuiltin,
        sh_restricted,
    },
    help::builtin_help,
};
...
pub fn enable_builtin(mut list: *mut WordList) -> i32 {
    ...
    if unsafe { restricted != 0 && flags & (FFLAG | DFLAG) != 0 } {
    ...
    if list.is_null() || flags & PFLAG != 0 {
        ...
        while !list.is_null() {
            opt = unsafe { dyn_unload_builtin((*(*list).word).word) };
...
fn list_some_builtins(mut filter: libc::c_int) {
    let mut i: i32 = 0;
    unsafe {
        while i < num_shell_builtins {
            ...
            if !(tmpIter.function.is_none() || tmpIter.flags & BUILTIN_DELETED != 0) {
                    ...
                    && (*shell_builtins.offset(i as isize)).flags & SPECIAL_BUILTIN == 0)
                {
                    ...
                    {
                        let name = CStr::from_ptr((*shell_builtins.offset(i as isize)).name);
...
fn enable_shell_command(mut name: *mut libc::c_char, mut disable_p: libc::c_int) -> libc::c_int {
    ...
    unsafe {
        ...
        while !list.is_null() {
            ...
            if b.is_null() {
                ...
                funcname = libc::realloc(
                    struct_name as *mut libc::c_void,
                    (size as usize)
                        .wrapping_add(::std::mem::size_of::<[libc::c_char; 14]>() as usize)
                ...
                loadfunc = ::std::mem::transmute::<*mut libc::c_void, sh_load_func_t>(dlsym(
                    handle, funcname,
                ));
                if loadfunc.is_some() {
        ...
        if new != 0 {
            ...
            size = ((total + 1 as libc::c_int) as libc::c_ulong)
                ...
                as libc::c_int;
            new_shell_builtins = libc::malloc(size as usize) as *mut builtin;
            libc::memcpy(
            ...
            while replaced < new {
                libc::memcpy(
                    &mut *new_shell_builtins.offset((num_shell_builtins + replaced) as isize)
                        as *mut builtin as *mut libc::c_char
                        as *mut libc::c_void,
...
fn delete_builtin(mut b: *mut builtin) {
...
fn local_dlclose(mut handle: *mut libc::c_void) -> libc::c_int {
    unsafe {
        return dlclose(handle);
    }
}
fn dyn_unload_builtin(mut name: *mut libc::c_char) -> libc::c_int {
    let mut b: *mut builtin = 0 as *mut builtin;
    let mut handle: *mut libc::c_void = 0 as *mut libc::c_void;
    let mut funcname: *mut libc::c_char = 0 as *mut libc::c_char;
    ...
    unsafe {
        ...
        size = strlen(name) as libc::c_int;
        funcname = libc::malloc(
            (size as usize)
                .wrapping_add(::std::mem::size_of::<[libc::c_char; 16]>() as usize)
                .wrapping_add(1 as libc::c_int as usize),
        ) as *mut libc::c_char;
        strcpy(funcname, name);
        strcpy(
        ...
        unloadfunc =
            ::std::mem::transmute::<*mut libc::c_void, sh_unload_func_t>(dlsym(handle, funcname));
