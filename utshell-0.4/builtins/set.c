...
extern int r_set_builtin PARAMS((WORD_LIST *));
extern int r_unset_builtin PARAMS((WORD_LIST *));
...
#define N_O_OPTIONS	(sizeof (o_optionsd) / sizeof (o_optionsd[0]))

#define GET_BINARY_O_OPTION_VALUE(i, name) \
  ((o_optionsd[i].get_func) ? (*o_optionsd[i].get_func) (name) \
			   : (*o_optionsd[i].variable))

#define SET_BINARY_O_OPTION_VALUE(i, onoff, name) \
  ((o_optionsd[i].set_func) ? (*o_optionsd[i].set_func) (onoff, name) \
			   : (*o_optionsd[i].variable = (onoff == FLAG_ON)))
     ...
     char *name;
{
  ...
  for (i = 0; o_optionsd[i].name; i++)
    if (STREQ (name, o_optionsd[i].name))
      return i;
  return -1;
}
     ...
     char *name;
{
  ...
  if (o_optionsd[i].letter)
    {
      on_or_off = find_flag (o_optionsd[i].letter);
     ...
     int mode, reusable;
{
  ...
  for (i = 0; o_optionsd[i].name; i++)
  {
      if (o_optionsd[i].letter)
        ...
        {
         //printf("now i  is  %d, value  is  %d, gflag  is %d \n, ",i,*on_or_off , reusable );
         print_minus_o_option (o_optionsd[i].name, *on_or_off, reusable);
        }
	        
	    }
      else{
	      value = GET_BINARY_O_OPTION_VALUE (i, o_optionsd[i].name);
	      if (mode == -1 || mode == value)
        {
          //printf("now ==== i  is  %d,value  is  %d gflag is %d \n",i,value ,reusable);
	        print_minus_o_option (o_optionsd[i].name, value, reusable);
        }
	    }
  }
}
...
get_current_options ()
{
  ...
  for (i = 0; o_optionsd[i].name; i++)
    {
      if (o_optionsd[i].letter)
	temp[i] = *(find_flag (o_optionsd[i].letter));
      else
	temp[i] = GET_BINARY_O_OPTION_VALUE (i, o_optionsd[i].name);
    }
     ...
     const char *bitmap;
{
  ...
  for (i = 0; o_optionsd[i].name; i++)
      ...
      if (o_optionsd[i].letter)
	{
	  ...
	  if (v != cv)
	    change_flag (o_optionsd[i].letter, v);
	}
      else
	{
	  cv = GET_BINARY_O_OPTION_VALUE (i, o_optionsd[i].name);
	  cv = cv ? FLAG_ON : FLAG_OFF;
	  if (v != cv)
	    SET_BINARY_O_OPTION_VALUE (i, v, o_optionsd[i].name);
	}
    }
     ...
     char *option_name;
{
  ...
  //printf ("option_name is  %s",option_name);
  //printf("i  is  %d",i);
  if (i < 0)
    {
  ...
  if (o_optionsd[i].letter == 0)
    {
      previous_option_value = GET_BINARY_O_OPTION_VALUE (i, o_optionsd[i].name);
      SET_BINARY_O_OPTION_VALUE (i, on_or_off, option_name);
      return (EXECUTION_SUCCESS);
    }
  else
    {
      if ((previous_option_value = change_flag (o_optionsd[i].letter, on_or_off)) == FLAG_ERROR)
	{
	  sh_invalidoptname (option_name);
...
set_shellopts ()
{
  ...
  for (vsize = i = 0; o_optionsd[i].name; i++)
    {
      tflag[i] = 0;
      if (o_optionsd[i].letter)
	{
	  ip = find_flag (o_optionsd[i].letter);
	  if (ip && *ip)
	    {
	      vsize += strlen (o_optionsd[i].name) + 1;
    //printf("1111 now  i is %d vsize is %d\n",i,vsize);
	      tflag[i] = 1;
	    }
	}
      else if (GET_BINARY_O_OPTION_VALUE (i, o_optionsd[i].name))
	{
	  vsize += strlen (o_optionsd[i].name) + 1;
    //printf("22222 now i is %d vsize is %d",i,vsize);
  ...
  for (i = vptr = 0; o_optionsd[i].name; i++)
    {
      if (tflag[i])
	{
	  strcpy (value + vptr, o_optionsd[i].name);
	  vptr += strlen (o_optionsd[i].name);
	  value[vptr++] = ':';
	}
    }
...
set_builtin (list)
     WORD_LIST *list;
{
  //printf("def set_builtin \n");
  return r_set_builtin(list);  
  ...
  while ((flag_name = internal_getopt (list, optflags)) != -1)
  {
      ...
      switch (flag_name){
	      ...
	      CASE_HELPOPT;
	      case '?':
          printf("entr show  help\n");
	        builtin_usage ();
	        return (list_optopt == '?' ? EXECUTION_SUCCESS : EX_USAGE);
	      default:
           printf("entr default help\n");
	          break;
	    }
  }
  ...
  for (force_assignment = opts_changed = 0; list; )
    {
    printf("force_assignment = opts_changed = 0 \n");
      arg = list->word->word;
    ...
    {
       printf("arg[0] == '-' %d  && (!arg[1]  %d || (arg[1] == '-' %d && !arg[2] %d)) \n",
  ...
  printf("on_or_off is  %d", *arg);
  if ((on_or_off = *arg) && (on_or_off == '-' || on_or_off == '+'))
	{
    printf("(on_or_off = *arg) && (on_or_off == '-' || on_or_off == '+')\n");
	  while (flag_name = *++arg){
        printf("flag_name = *++arg \n");
	      if (flag_name == '?')
         {
		      builtin_usage ();
		      return (EXECUTION_SUCCESS);
        ...
        else if (flag_name == 'o') /* -+o option-name */
        {
          ...
          printf("option_name ==== \n");
            ...
            printf(" option_name == 0 || *option_name == '\0' \n");
		        list_minus_o_opts (-1, (on_or_off == '+'));
		        continue;
		      }
		    ...
		    {
          printf("change_flag (flag_name, on_or_off) == FLAG_ERROR \n"); 
		      s[0] = on_or_off;
		      s[1] = flag_name;
		      s[2] = '\0';
		      sh_invalidopt (s);
...
unset_builtin (list)
  WORD_LIST *list;
{
  //printf("enter unset opt\n");
  return r_unset_builtin(list);
