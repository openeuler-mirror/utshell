...
pub union REDIRECTEE {
    pub dest: libc::c_int,       /* Place to redirect REDIRECTOR to, or ... */
    pub filename: *mut WordDesc, /* filename to redirect to. */
}
...
pub extern "C" fn r_set_dollar_vars_changed() {
    unsafe {
        ...
        } else if this_shell_builtin == set_builtin as *mut sh_builtin_func_t {
...
#[no_mangle]
pub extern "C" fn r_get_numeric_arg(
...
pub extern "C" fn r_get_exitstat(mut list: *mut WordList) -> i32 {
    ...
    unsafe {
        ...
        if list.is_null() {
            ...
            if this_shell_builtin == return_builtin as *mut sh_builtin_func_t
                && running_trap > 0
        ...
        while !list.is_null() {
            if legal_number((*(*list).word).word, &mut lsignum) != 0 {
                ...
                if posixly_correct == 0
                    || this_shell_builtin != kill_builtin as *mut sh_builtin_func_t
                {
...
pub extern "C" fn r_initialize_shell_builtins() {
    unsafe {
        qsort(
            ...
            size_of::<builtin>(),
            std::mem::transmute::<Option<unsafe extern "C" fn() -> libc::c_int>, Option<QSFUNC>>(
...
/* **************************************************************** */
#[no_mangle]
pub extern "C" fn r_builtin_bind_variable(
    name: *mut c_char,
    value: *mut c_char,
    flags: i32,
) -> *mut SHELL_VAR {
