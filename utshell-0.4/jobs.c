...
// 令 buf 为 NULL并使 size 为0来使 getcwd 调用 malloc 动态给 buf 分配,但是这种情况要特别注意使用后释放缓冲以防止内存泄漏。
...
//清理job和扩容。
  ...
  //  两个数据结构js.j_jobslots，和 jobs 。先设置j_jobslots=8,js.j_firstj ,j_lastj j_njobs设置为0
  if (js.j_jobslots == 0)
    {
...
// 新建job块，
  ...
  if (async)
    ...
    // 有job_control,进程组id不为0,同步的，不是运行在后台的，才会给terminal
     ...
     pid_t pid;
{
  ...
  return (&pidstat_table[hash % PIDSTAT_TABLE_SZ]);    //4096
}
     ...
     int job_index, dflags;
{
  ...
  free (temp->wd);
  ndel = discard_pipeline (temp->pipe);//清理了proc
...
// 设置终端的tty,给进程组pgrp，需要阻塞所有终端的信号SIGTTOU，SIGTTIN，SIGTSTP，SIGCHLD
     ...
     int flags;
{
  ...
  tpgrp = tcgetpgrp (shell_tty);//获得与tty关联的进城组id
  if (tpgrp < 0 && errno == ENOTTY)
    return -1;
  if (tpgrp == npgrp)
    {
      //如果获得的组id是传入参数中新的组id，就把terminal_pgrp赋值成新的，然后返回
      terminal_pgrp = npgrp;
      return 0;
    }
  else if (tpgrp != opgrp)
    ...
    // else分支中，获得的pgrp必须是老的，因为不是已经被返回了。
  else
...
#endif
      if (jobs[i] && DEADJOB (i))//判断jobs[i].state==JDEAD
	{
	  ndead++;//死job的个数
	  ndeadproc += processes_in_job (i);//返回job[i]->pipe的个数，死进程的个数，job可以包含多个进程.
	}
    }
...
itrace("mark_dead_jobs_as_notified: child_max = %d ndead = %d ndeadproc = %d", js.c_childmax, ndead, ndeadproc);
  ...
  for (i = 0; i < js.j_jobslots; i++)
    {
      // job存在，且死了，，在interactive环境，每个job最后一个进程不是last_asynchronous_pid
      if (jobs[i] && DEADJOB (i) && (interactive_shell || (find_last_pid (i, 0) != last_asynchronous_pid)))
	{
#if defined (DEBUG)
	  if (i < js.j_firstj && jobs[i])
	  ...
	  /*
如果将此作业标记为已通知，会使我们下降到child_max以下，请不要标记它，这样我们至少可以保持child_max状态。XXX——需要检查Posix关于保持状态的实际说法。
	   */
