...
[cd]
    改变 shell 工作目录。
        ...
        将当前目录更改为DIR。默认的DIR是HOME shell变量的值。如果DIR为“-”，
        则转换为$OLDPWD。
        
        变量CDPATH定义了包含DIR的目录的搜索路径。CDPATH中的备选目录名以冒号（:）分隔。
        空目录名称与当前目录名称相同。如果DIR以斜杠（/）开头，则不使用CDPATH。
        
        如果没有找到该目录，并且设置了shell选项‘ cdable_vars’，则假定该单词是一个变量名。
        如果该变量有一个值，则将其值用于DIR。
...
[history] 
    显示或操纵历史列表。
        ...
        有值的话，则使用它，否则使用 ~/.utshell_history 文件。
...
[let] 
    估值算术表达式。
        ...
        将每个 参数 作为算术表达式进行求值。求值在固定宽度的整
        数中完成，没有溢出检测，不过除 0 的异常会被捕获，并且会报一个错
        误。下面的运算符列表中，同一行的运算符的优先级相等。列表按照
        优先级从高至低排序。 
