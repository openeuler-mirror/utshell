...
fn c_getopt(
    ...
    unsafe { getopt(___argc, ___argv, __shortopts) }
}

fn c_strcpy(a: *mut libc::c_char, b: *const libc::c_char) -> *mut libc::c_char {
    unsafe { strcpy(a, b) }
}

fn c_strchr(a: *const libc::c_char, b: libc::c_int) -> *mut libc::c_char {
    unsafe { strchr(a, b) }
}

fn c_strrchr(a: *const libc::c_char, b: libc::c_int) -> *mut libc::c_char {
    unsafe { strrchr(a, b) }
}

fn c_exit(a: libc::c_int) -> ! {
    unsafe { exit(a) }
}
...
unsafe fn main_0(mut argc: libc::c_int, mut argv: *mut *mut libc::c_char) -> libc::c_int {
    ...
    progname = c_strrchr(*argv.offset(0 as libc::c_int as isize), '/' as i32);
    if !progname.is_null() {
    ...
    if oflags & (RFLAG | VFLAG) != 0 {
        c_strcpy(dv.as_mut_ptr(), dist_version);
        rv = c_strchr(dv.as_mut_ptr(), '.' as i32);
        if !rv.is_null() {
            let fresh0 = rv;
            rv = rv.offset(1);
