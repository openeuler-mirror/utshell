...
use libc::{c_char, c_int, c_long, c_void, WEXITSTATUS, WIFSIGNALED, WIFSTOPPED, WTERMSIG};
use r_bash::*;
...
#[no_mangle]
pub static mut check_window_size: libc::c_int = CHECKWINSIZE_DEFAULT as libc::c_int;
#[no_mangle]
pub static mut job_control: libc::c_int = 0;
#[no_mangle]
...
#[no_mangle]
pub static mut pid_list_size: libc::c_int = 0;
#[no_mangle]
pub static mut wait_sigint_received: libc::c_int = 0;
#[no_mangle]
pub static mut child_max: linc::c_long = -1;

unsafe extern "C" fn alloc_pid_list() {
    let mut i: libc::c_int;
    let mut old: libc::c_int = pid_list_size;

    pid_list_size += 10;
    pid_list = libc::realloc(
        pid_list as *mut c_void,
        (pid_list_size * size_of::<proc_status>() as libc::c_int) as usize,
    ) as *mut proc_status;
...
unsafe extern "C" fn find_proc_slot(pid: pid_t) -> libc::c_int {
    let mut i: libc::c_int;
...
unsafe extern "C" fn find_index_by_pid(pid: pid_t) -> libc::c_int {
    let mut i: libc::c_int;
...
unsafe extern "C" fn process_exit_status(status: WAIT) -> libc::c_int {
...
unsafe extern "C" fn find_termsig_by_pid(pid: pid_t) -> libc::c_int {
    let mut i: libc::c_int;
...
unsafe extern "C" fn get_termsig(status: WAIT) -> libc::c_int {
    if WIFSTOPPED(status) as libc::c_int == 0 && WIFSIGNALED(status) {
...
unsafe extern "C" fn set_pid_flags(pid: pid_t, flags: libc::c_int) {
    let mut slot: libc::c_int;
...
unsafe extern "C" fn unset_pid_flags(pid: pid_t, flags: libc::c_int) {
    let mut slot: libc::c_int;
...
unsafe extern "C" fn get_pid_flags(pid: pid_t) -> libc::c_int {
    let mut slot: libc::c_int;
    slot = find_index_by_pid(pid);
    if slot == NO_PID!() {
        return 0;
...
unsafe extern "C" fn add_pid(pid: pid_t, async_: libc::c_int) {
    let mut slot: libc::c_int;
...
unsafe extern "C" fn mark_dead_jobs_as_notified(force: libc::c_int) {
    let mut i: libc::c_int;
    ...
    if force == 0 && ndead <= child_max as libc::c_int {
        return;
    }
