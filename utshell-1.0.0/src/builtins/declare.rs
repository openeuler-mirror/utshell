...
pub fn local_builtin(list: *mut WordList) -> i32 {
    ...
    if unsafe {
        ...
        list != std::ptr::null_mut()
            ...
            && STREQ((*(*list).word).word, msg.as_ptr())
    } {
        builtin_help();
        return EX_USAGE!();
    }

    if unsafe { variable_context != 0 } {
        return declare_internal(list, 1);
    } else {
        unsafe {
            let msg = CString::new("can only be used in a function").unwrap();
...
pub fn declare_internal(mut list: *mut WordList, local_var: i32) -> i32 {
    ...
    'outter: while list != std::ptr::null_mut() {
        ...
        let array_exists: i32;
        let creating_array: i32;
        ...
        if unsafe { local_var != 0 && variable_context != 0 && STREQ(name, msg.as_ptr()) } {
            var = make_local_variable(msg.as_ptr(), 0);
            if value_cell(var) != std::ptr::null_mut() {
                unsafe {
                    libc::free(value_cell(var) as *mut c_void);
                } /* just in case */
            }
        ...
        if (flags_on & att_nameref!()) != 0 {
            if valid_array_reference(name, 0) != 0 {
                let msg = CString::new("%s: reference variable cannot be an array").unwrap();
                unsafe {
                    builtin_error(msg.as_ptr(), name);
                    assign_error += 1;
                    libc::free(name as *mut c_void);
                    list = (*list).next;
                }
                ...
                unsafe {
                    let msg =
                        CString::new("%s: nameref variable self references not allowed").unwrap();
                    if variable_context == 0 {
                        ...
                        let msg = CString::new("%s: circular name reference").unwrap();
                        builtin_warning(msg.as_ptr(), name);
                    }
                }
            }
                ...
                && valid_nameref_value(value, 1) == 0
            {
                unsafe {
                    let msg =
                        CString::new("`nvalid %s': ivariable name for name reference").unwrap();
                    builtin_error(msg.as_ptr(), value);
                    assign_error += 1;
                    libc::free(name as *mut c_void);
                    list = (*list).next;
                }
        ...
        'inner: loop {
            ...
            if unsafe {
                (posixly_correct != 0 || (flags_on & att_function!()) == 0)
            ...
            refvar = std::ptr::null_mut();
            if unsafe {
                variable_context != 0 && mkglobal == 0 && ((flags_on & att_function!()) == 0)
            } {
                ...
                if var == std::ptr::null_mut() {
                    ...
                    if refvar != std::ptr::null_mut()
                        && unsafe { (*refvar).context != variable_context }
                    {
                        newname = name;
                    } else {
                        unsafe {
            ...
            if (flags_on & att_function!()) != 0 {
                if offset != 0 {
                    ...
                    unsafe {
                        let msg = CString::new("cannot use `-f' to make functions").unwrap();
                        builtin_error(msg.as_ptr());
                        libc::free(name as *mut c_void);
                    }
                    return EXECUTION_FAILURE!();
                    ...
                    if var != std::ptr::null_mut() {
                        if readonly_p(var) != 0 && (flags_off & att_readonly!()) != 0 {
                            unsafe {
                                let msg = CString::new("%s: readonly function").unwrap();
                                builtin_error(msg.as_ptr(), name);
                                any_failed += 1;
                                libc::free(name as *mut c_void);
                                list = (*list).next;
                        ...
                        if flags_on == att_function!() && flags_off == 0 {
                            if unsafe { nodefs != 0 && debugging_mode != 0 } {
                                unsafe {
                ...
                if var == std::ptr::null_mut() && (flags_on & att_nameref!()) != 0 {
                    ...
                    *CHANGING ITS VALUE AS A SIDE EFFECT*, then turn off the nameref
                    ...
                    if var != std::ptr::null_mut() && nameref_p(var) != 0 {
                        unsafe {
                            let msg = CString::new("%s: removing nameref attribute").unwrap();
                            internal_warning(msg.as_ptr(), name);
                    ...
                    && (flags_on & att_nameref!()) != 0
                {
                    ...
                    unsafe {
                        let msg =
                            CString::new("%s: reference variable cannot be an array").unwrap();
                        builtin_error(msg.as_ptr(), name);
                        assign_error += 1;
                        libc::free(name as *mut c_void);
                        list = (*list).next;
                    ...
                    && valid_nameref_value(value, 1) == 0
                {
                    unsafe {
                        let msg =
                            CString::new("`%s': invalid variable name for name reference").unwrap();
                        builtin_error(msg.as_ptr(), value);
                        any_failed += 1;
                        libc::free(name as *mut c_void);
                        list = (*list).next;
                        ...
                        unsafe {
                            let msg =
                                CString::new("`%s': invalid variable name for name reference")
                                    .unwrap();
                            builtin_error(msg.as_ptr(), value_cell(var));
                            any_failed += 1;
                            libc::free(name as *mut c_void);
                            list = (*list).next;
                        }
                ...
                } else if (making_array_special != 0 || creating_array != 0 || array_exists != 0)
                    && offset != 0
                {
                    let vlen: i32;
                    unsafe {
                            ...
                            unsafe {
                                let msg =
                                    CString::new("%s: quoted compound array assignment deprecated")
                                        .unwrap();
                                internal_warning(msg.as_ptr(), (*(*list).word).word);
                            }
                        }
                    ...
                    || ((flags_off & att_assoc!()) != 0 && assoc_p(var) != 0)
                {
                    unsafe {
                        let msg =
                            CString::new("%s: cannot destroy array variables in this way").unwrap();
                        builtin_error(msg.as_ptr(), name);
                        any_failed += 1;
                        libc::free(name as *mut c_void);
                        list = (*list).next;
                ...
                if (flags_on & att_array!()) != 0 && assoc_p(var) != 0 {
                    unsafe {
                        let msg = CString::new("%s: cannot convert associative to indexed array")
                            .unwrap();
                        builtin_error(msg.as_ptr(), name);
                        any_failed += 1;
                        libc::free(name as *mut c_void);
                ...
                if (flags_on & att_assoc!()) != 0 && array_p(var) != 0 {
                    unsafe {
                        let msg = CString::new("%s: cannot convert indexed to associative array")
                            .unwrap();
                        builtin_error(msg.as_ptr(), name);
                        any_failed += 1;
                        libc::free(name as *mut c_void);
                ...
                if offset != 0 && compound_array_assign != 0 {
                    ...
                    if assoc_p(var) != 0 {
                        unsafe {
                            let msg = CString::new("0").unwrap();
                            bind_assoc_variable(
                                var,
                                name,
                                savestring!(msg.as_ptr()),
                                value,
                                aflags | ASS_FORCE!(),
                            );
                        }
