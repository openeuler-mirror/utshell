...
pub fn fg_builtin(list: *mut WordList) -> i32 {
    ...
    if unsafe { loptend == std::ptr::null_mut() } {
        return unsafe { fg_bg(loptend, 1) };
    } else {
        let mut t: WordList = unsafe { *loptend };
        while t.next != std::ptr::null_mut() {
            t = unsafe { *(t.next) };
        }
        let cstr: &std::ffi::CStr = unsafe { std::ffi::CStr::from_ptr((*(t.word)).word) };
        ...
        return unsafe { fg_bg(loptend, fg_bit) };
    }
}
...
pub fn bg_builtin(list: *mut WordList) -> i32 {
    ...
    if unsafe { fg_bg(loptend, 0) == EXECUTION_FAILURE!() } {
        r = EXECUTION_FAILURE!();
    }
...
pub fn fg_bg(list: *mut WordList, foreground: i32) -> i32 {
    ...
    j = unsafe { get_job_by_jid!(job) };
    /* Or if j->pgrp == shell_pgrp. */
    if unsafe { !IS_JOBCONTROL!(job) } {
