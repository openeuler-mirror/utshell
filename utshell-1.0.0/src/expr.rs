...
#[no_mangle]
fn expr_bind_variable(lhs: *mut libc::c_char, rhs: *mut libc::c_char) {
    let v: *mut SHELL_VAR;
    let aflags: libc::c_int;

    if unsafe { lhs.is_null() || *lhs as libc::c_int == 0 as libc::c_int } {
        return;
    }
    aflags = if unsafe { assoc_expand_once != 0 && already_expanded != 0 } {
    ...
    if unsafe {
        ...
        unsafe {
            c_siglongjmp(evalbuf.as_mut_ptr(), 1 as libc::c_int);
        }
    }
...
#[no_mangle]
fn expr_streval(tok: *mut libc::c_char, e: libc::c_int, lvalue: *mut lvalue) -> intmax_t {
    let mut v: *mut SHELL_VAR;
    let mut value: *mut libc::c_char;
    let tval: intmax_t;
    let initial_depth: libc::c_int;
    let mut ind: arrayind_t;
    let tflag: libc::c_int;
    let aflag: libc::c_int;
    ...
    if unsafe {
        (v.is_null() || (*v).attributes & 0x1000 as libc::c_int != 0) && unbound_vars_is_error != 0
    } {
        value = if e == ']' as i32 {
            array_variable_name(
                tok,
    ...
    tval = if unsafe { !value.is_null() && *value as libc::c_int != 0 } {
        subexpr(value)
    } else {
...
#[no_mangle]
fn expr_bind_array_element(tok: *mut libc::c_char, ind: arrayind_t, rhs: *mut libc::c_char) {
    let lhs: *mut libc::c_char;
    let vname: *mut libc::c_char;
    let llen: size_t;
    let mut ibuf: [libc::c_char; 22] = [0; 22];
    let istr: *mut libc::c_char;
    ...
    unsafe {
        llen = (strlen(vname))
            .wrapping_add(::std::mem::size_of::<[libc::c_char; 22]>())
...
fn exppower() -> intmax_t {
    let mut val1: intmax_t;
    let mut val2: intmax_t;
    // let c: intmax_t = 0;
    val1 = exp1();

    while unsafe { curtok == POWER as libc::c_int } {
        readtok();
        val2 = exppower();
        unsafe {
...
fn expmuldiv() -> intmax_t {
    let mut val1: intmax_t;
    let mut val2: intmax_t;
    let mut idiv: imaxdiv_t;
    val1 = exppower();

    while unsafe { curtok == '*' as i32 || curtok == '/' as i32 || curtok == '%' as i32 } {
        let op: libc::c_int = unsafe { curtok };
        let stp: *mut libc::c_char;
        let sltp: *mut libc::c_char;
        stp = unsafe { tp };
        readtok();
        val2 = exppower();
        if (op == '/' as i32 || op == '%' as i32) && val2 == 0 as libc::c_int as libc::c_long {
            if unsafe { noeval == 0 as libc::c_int } {
                sltp = unsafe { lasttp };
                unsafe {
                    lasttp = stp;
                }
...
fn exp3() -> intmax_t {
    ...
    while unsafe { curtok == '+' as i32 || curtok == '-' as i32 } {
        let op: libc::c_int = unsafe { curtok };
        readtok();
        val2 = expmuldiv();
        if op == '+' as i32 {
...
fn expshift() -> intmax_t {
    ...
    while unsafe { curtok == LSH as libc::c_int || curtok == RSH as libc::c_int } {
        let op: libc::c_int = unsafe { curtok };
        readtok();
        val2 = exp3();
...
fn exp5() -> intmax_t {
    ...
    while unsafe { curtok == EQEQ as libc::c_int || curtok == NEQ as libc::c_int } {
        let op: libc::c_int = unsafe { curtok };
        readtok();
        val2 = exp4();
...
fn expland() -> intmax_t {
    let mut val1: intmax_t;
    let mut val2: intmax_t;
    let mut set_noeval: libc::c_int;
    val1 = expbor();

    while unsafe { curtok == LAND as libc::c_int } {
...
fn explor() -> intmax_t {
    ...
    while unsafe { curtok == LOR as libc::c_int } {
        set_noeval = 0 as libc::c_int;
        if val1 != 0 as libc::c_int as libc::c_long {
            unsafe {
                noeval += 1;
...
fn expcond() -> intmax_t {
    ...
    if unsafe { curtok == '?' as i32 } {
        ...
        readtok();
        if unsafe { curtok == 0 as libc::c_int || curtok == ':' as i32 } {
            evalerror(c_dcgettext(
                0 as *const libc::c_char,
        ...
        readtok();
        if unsafe { curtok == 0 as libc::c_int } {
            evalerror(c_dcgettext(
                0 as *const libc::c_char,
                b"expression expected\0" as *const u8 as *const libc::c_char,
                5 as libc::c_int,
...
fn expassign() -> intmax_t {
    ...
    if unsafe { curtok == '=' as i32 || curtok == 11 as libc::c_int } {
        let special: libc::c_int;
        let mut op: libc::c_int = 0;
        let mut lvalue: intmax_t = 0;
        special = unsafe { (curtok == 11 as libc::c_int) as libc::c_int };
        if unsafe { lasttok != 5 as libc::c_int } {
            evalerror(c_dcgettext(
                0 as *const libc::c_char,
        ...
        lhs = unsafe {
            strcpy(
                libc::malloc((1 as usize).wrapping_add(strlen(tokstr))) as *mut libc::c_char,
                tokstr,
            )
        };
        lind = unsafe { curlval.ind };
        readtok();
        value = expassign();
        if special != 0 {
...
#[no_mangle]
fn strlong(num: *mut libc::c_char) -> intmax_t {
    let mut s: *mut libc::c_char;
    let mut c: libc::c_uchar;
    let mut base: libc::c_int;
    let mut foundbase: libc::c_int;
    let mut val: intmax_t;
    s = num;
    ...
    while c != 0 {
        ...
        let fresh15 = s;
        s = unsafe { s.offset(1) };
        c = unsafe { *fresh15 as libc::c_uchar };
    }
...
#[no_mangle]
fn expr_skipsubscript(vp: *mut libc::c_char, cp: *mut libc::c_char) -> libc::c_int {
    ...
    if unsafe { assoc_expand_once & already_expanded != 0 } {
        unsafe {
    ...
    flags = if isassoc != 0 && unsafe { assoc_expand_once != 0 } && unsafe { already_expanded != 0 }
    {
        0x1 as libc::c_int
    } else {
        0 as libc::c_int
...
#[no_mangle]
fn evalerror(msg: *const libc::c_char) {
...
#[no_mangle]
fn _is_arithop(c: libc::c_int) -> libc::c_int {
...
#[no_mangle]
fn _is_multiop(c: libc::c_int) -> libc::c_int {
    match c {
...
fn readtok() {
    ...
    let mut xp: *mut libc::c_char;
    let mut c: libc::c_uchar;
    let c1: libc::c_uchar;
    let mut e: libc::c_int;
    // let lval: lvalue = lvalue {
    //     tokstr: 0 as *mut libc::c_char,
    //     tokval: 0,
    //     tokvar: 0 as *mut SHELL_VAR,
    //     ind: 0,
    ...
    unsafe {
            ...
            || c as libc::c_int == '_' as i32
        {
            ...
            *cp = '\u{0}' as i32 as libc::c_char;
            ...
            tokstr = strcpy(
                libc::malloc((1 as usize).wrapping_add(strlen(tp))) as *mut libc::c_char,
                tp,
            );
            *cp = c as libc::c_char;
            ec.curtok = curtok;
            ...
            *cp = c as libc::c_char;
                ...
                && c1 as libc::c_int == c as libc::c_int
            {
                ...
                if 1 as libc::c_int != 0
                    && *(*c___ctype_b_loc()).offset(*xp as libc::c_uchar as libc::c_int as isize)
                        as libc::c_int
                        & _ISalpha as libc::c_int as libc::c_ushort as libc::c_int
                        != 0
...
#[no_mangle]
fn subexpr(expr: *mut libc::c_char) -> intmax_t {
    let val: intmax_t;
    ...
    unsafe {
        ...
        pushexp();
        expression = strcpy(
            libc::malloc((1 as usize).wrapping_add(strlen(expr))) as *mut libc::c_char,
            expr,
        );
        tp = expression;
...
#[no_mangle]
pub fn evalexp(expr: *mut libc::c_char, flags: libc::c_int, validp: *mut libc::c_int) -> intmax_t {
    let val: intmax_t;
    let c: libc::c_int;
