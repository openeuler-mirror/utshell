...
#[no_mangle]
pub fn begin_unwind_frame(tag: *mut libc::c_char) {
    unsafe {
        add_unwind_protect(
...
#[no_mangle]
pub fn discard_unwind_frame(tag: *mut libc::c_char) {
    unsafe {
...
#[no_mangle]
pub fn run_unwind_frame(tag: *mut libc::c_char) {
    unsafe {
        if !unwind_protect_list.is_null() {
            without_interrupts(
                ::std::mem::transmute::<
...
#[no_mangle]
pub fn add_unwind_protect(cleanup: Option<Function>, arg: *mut libc::c_char) {
    unsafe {
        without_interrupts(
            ::std::mem::transmute::<
...
#[no_mangle]
pub fn clear_unwind_protect_list(flags: libc::c_int) {
    unsafe {
        let flag: *mut libc::c_char;
        if !unwind_protect_list.is_null() {
            flag = (if flags != 0 {
...
#[no_mangle]
pub fn unwind_protect_tag_on_stack(tag: *const libc::c_char) -> libc::c_int {
    let mut elt: *mut UNWIND_ELT;
    unsafe {
...
fn add_unwind_protect_internal(cleanup: Option<Function>, arg: *mut libc::c_char) {
    let elt: *mut UNWIND_ELT;
    unsafe {
...
fn remove_unwind_protect_internal(_ignore1: *mut libc::c_char, _ignore2: *mut libc::c_char) {
    let elt: *mut UNWIND_ELT;

    elt = unsafe { unwind_protect_list };
    if !elt.is_null() {
        unsafe {
            unwind_protect_list = (*unwind_protect_list).head.next;
        }
        if unsafe { uwcache.nc < uwcache.cs } {
            if ::std::mem::size_of::<UNWIND_ELT>() as libc::c_ulong
                <= 32 as libc::c_int as libc::c_ulong
            {
                let mut mzp: *mut libc::c_char = elt as *mut libc::c_char;
                let mut mctmp: libc::c_ulong = ::std::mem::size_of::<UNWIND_ELT>() as libc::c_ulong;
                let mut mcn: libc::c_long;
...
fn run_unwind_protects_internal(_ignore1: *mut libc::c_char, _ignore2: *mut libc::c_char) {
    unwind_frame_run_internal(
        0 as *mut libc::c_void as *mut libc::c_char,
        0 as *mut libc::c_void as *mut libc::c_char,
    );
}
fn clear_unwind_protects_internal(flag: *mut libc::c_char, _ignore: *mut libc::c_char) {
    if !flag.is_null() {
        while unsafe { !unwind_protect_list.is_null() } {
...
fn unwind_frame_discard_internal(tag: *mut libc::c_char, _ignore: *mut libc::c_char) {
    let mut elt: *mut UNWIND_ELT;
    ...
    loop {
        ...
        if unsafe {
            ...
            break;
        } else if unsafe { uwcache.nc < uwcache.cs } {
            if ::std::mem::size_of::<UNWIND_ELT>() as libc::c_ulong
                <= 32 as libc::c_int as libc::c_ulong
            {
...
#[inline]
fn restore_variable(sv: *mut SAVED_VAR) {
    unsafe {
        libc::memcpy(
...
fn unwind_frame_run_internal(tag: *mut libc::c_char, _ignore: *mut libc::c_char) {
    let mut elt: *mut UNWIND_ELT;
    let mut found: libc::c_int;
    found = 0 as libc::c_int;
    loop {
        ...
        if unsafe { ((*elt).head.cleanup).is_none() } {
            if unsafe {
                !tag.is_null()
                    && (*((*elt).arg.v).offset(0 as libc::c_int as isize) as libc::c_int
                        == *tag.offset(0 as libc::c_int as isize) as libc::c_int
...
fn unwind_protect_mem_internal(var: *mut libc::c_char, psize: *mut libc::c_char) {
    let size: libc::c_int;
    let mut allocated: libc::c_int;
    let elt: *mut UNWIND_ELT;
    unsafe {
...
#[no_mangle]
pub fn unwind_protect_mem(var: *mut libc::c_char, mut size: libc::c_int) {
    unsafe {
        without_interrupts(
