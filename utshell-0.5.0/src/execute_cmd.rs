...
to close. */
#[no_mangle]
pub fn new_fd_bitmap(mut size: libc::c_int) -> *mut fd_bitmap {
...
#[no_mangle]
pub fn dispose_fd_bitmap(mut fdbp: *mut fd_bitmap) {
    unsafe {
        FREE!((*fdbp).bitmap);
...
#[no_mangle]
pub fn close_fd_bitmap(mut fdbp: *mut fd_bitmap) {
...
#[no_mangle]
pub fn execute_command(mut command: *mut COMMAND) -> libc::c_int {
...
fn shell_control_structure(mut type_0: command_type) -> libc::c_int {
...
fn cleanup_redirects(mut list: *mut REDIRECT) {
    unsafe {
...
fn restore_signal_mask(mut set: *mut sigset_t) -> libc::c_int {
    unsafe {
...
#[no_mangle]
pub fn getcoprocbypid(mut pid: pid_t) -> *mut coproc {
    unsafe {
...
#[no_mangle]
pub fn getcoprocbyname(mut name: *const libc::c_char) -> *mut coproc {
    unsafe {
...
#[no_mangle]
pub fn coproc_init(mut cp: *mut coproc) {
    unsafe {
...
#[no_mangle]
pub fn coproc_alloc(mut name: *mut libc::c_char, mut pid: pid_t) -> *mut coproc {
...
#[no_mangle]
pub fn coproc_dispose(mut cp: *mut coproc) {
...
#[no_mangle]
pub fn coproc_close(mut cp: *mut coproc) {
    unsafe {
        if (*cp).c_rfd >= 0 {
            close((*cp).c_rfd);
            (*cp).c_rfd = -1;
...
#[no_mangle]
pub fn coproc_rclose(mut cp: *mut coproc, mut fd: libc::c_int) {
    unsafe {
...
#[no_mangle]
pub fn coproc_wclose(mut cp: *mut coproc, mut fd: libc::c_int) {
    unsafe {
...
#[no_mangle]
pub fn coproc_checkfd(mut cp: *mut coproc, mut fd: libc::c_int) {
...
#[no_mangle]
pub fn coproc_fdchk(mut fd: libc::c_int) {
    unsafe {
...
#[no_mangle]
pub fn coproc_fdclose(mut cp: *mut coproc, mut fd: libc::c_int) {
    coproc_rclose(cp, fd);
    coproc_wclose(cp, fd);
    coproc_setvars(cp);
}
...
#[no_mangle]
pub fn coproc_fdsave(mut cp: *mut coproc) {
    unsafe {
        (*cp).c_rsave = (*cp).c_rfd;
        (*cp).c_wsave = (*cp).c_wfd;
    }
...
#[no_mangle]
pub fn coproc_fdrestore(mut cp: *mut coproc) {
    unsafe {
        (*cp).c_rfd = (*cp).c_rsave;
        (*cp).c_wfd = (*cp).c_wsave;
    }
...
fn coproc_setstatus(mut cp: *mut coproc, mut status: libc::c_int) {
    unsafe {
        (*cp).c_lock = 4;
        (*cp).c_status = status;
...
#[no_mangle]
pub fn coproc_pidchk(mut pid: pid_t, mut status: libc::c_int) {
    let mut cp: *mut coproc = 0 as *mut coproc;
...
#[no_mangle]
pub fn coproc_setvars(mut cp: *mut coproc) {
...
#[no_mangle]
pub fn coproc_unsetvars(mut cp: *mut coproc) {
    let mut l: libc::c_int = 0;
    let mut namevar: *mut libc::c_char = 0 as *mut libc::c_char;
    unsafe {
        if ((*cp).c_name).is_null() {
...
fn restore_stdin(mut s: libc::c_int) {
    unsafe {
        dup2(s, 0);
        close(s);
    }
}
fn lastpipe_cleanup(mut s: libc::c_int) {
    unsafe {
...
fn execute_for_command(mut for_command: *mut FOR_COM) -> libc::c_int {
...
fn eval_arith_for_expr(mut l: *mut WordList, mut okp: *mut libc::c_int) -> intmax_t {
...
fn execute_arith_for_command(mut arith_for_command: *mut ARITH_FOR_COM) -> libc::c_int {
    let mut expresult: intmax_t = 0;
...
fn displen(mut s: *const libc::c_char) -> libc::c_int {
...
fn indent(mut from: libc::c_int, mut to: libc::c_int) {
    unsafe {
        while from < to {
...
fn execute_select_command(mut select_command: *mut SELECT_COM) -> libc::c_int {
    let mut releaser: *mut WordList = 0 as *mut WordList;
    let mut list: *mut WordList = 0 as *mut WordList;
    let mut v: *mut SHELL_VAR = 0 as *mut SHELL_VAR;
    let mut identifier: *mut libc::c_char = 0 as *mut libc::c_char;
...
fn execute_case_command(mut case_command: *mut CASE_COM) -> libc::c_int {
    let mut list: *mut WordList = 0 as *mut WordList;
...
fn execute_while_command(mut while_command: *mut WHILE_COM) -> libc::c_int {
...
fn execute_until_command(mut while_command: *mut WHILE_COM) -> libc::c_int {
...
fn execute_if_command(mut if_command: *mut IF_COM) -> libc::c_int {
    let mut return_value: libc::c_int = 0;
    let mut save_line_number: libc::c_int = 0;
    unsafe {
        save_line_number = line_number;
...
fn execute_arith_command(mut arith_command: *mut ARITH_COM) -> libc::c_int {
    let mut expok: libc::c_int = 0;
    let mut save_line_number: libc::c_int = 0;
    let mut retval: libc::c_int = 0;
    let mut expresult: intmax_t = 0;
...
fn execute_cond_node(mut cond: *mut COND_COM) -> libc::c_int {
    let mut result: libc::c_int = 0;
    let mut invert: libc::c_int = 0;
...
fn execute_cond_command(mut cond_command: *mut COND_COM) -> libc::c_int {
    let mut retval: libc::c_int = 0;
    let mut save_line_number: libc::c_int = 0;
    unsafe {
        save_line_number = line_number;
...
fn fix_assignment_words(mut words: *mut WordList) {
...
fn check_command_builtin(mut words: *mut WordList, mut typep: *mut libc::c_int) -> *mut WordList {
...
fn is_dirname(mut pathname: *mut libc::c_char) -> libc::c_int {
...
fn builtin_status(mut result: libc::c_int) -> libc::c_int {
    let mut r: libc::c_int = 0;
...
fn maybe_restore_getopt_state(mut gs: *mut sh_getopt_state_t) {
    unsafe {
        if (*gs).gs_flags & 1 != 0 {
...
#[no_mangle]
pub fn restore_funcarray_state(mut fa: *mut func_array_state) {
...
#[no_mangle]
pub fn execute_shell_function(mut var: *mut SHELL_VAR, mut words: *mut WordList) -> libc::c_int {
    let mut ret: libc::c_int = 0;
    let mut bitmap: *mut fd_bitmap = 0 as *mut fd_bitmap;
    unsafe {
        bitmap = new_fd_bitmap(FD_BITMAP_DEFAULT_SIZE!());
...
fn execute_intern_function(mut name: *mut WordDesc, mut funcdef: *mut FUNCTION_DEF) -> libc::c_int {
    let mut var: *mut SHELL_VAR = 0 as *mut SHELL_VAR;
    let mut t: *mut libc::c_char = 0 as *mut libc::c_char;
    unsafe {
...
fn close_pipes(mut in_0: libc::c_int, mut out: libc::c_int) {
    unsafe {
        if in_0 >= 0 {
            close(in_0);
        }
...
fn dup_error(mut oldd: libc::c_int, mut newd: libc::c_int) {
    unsafe {
        sys_error(
...
fn do_piping(mut pipe_in: libc::c_int, mut pipe_out: libc::c_int) {
    unsafe {
        if pipe_in != NO_PIPE {
            if dup2(pipe_in, 0) < 0 {
                dup_error(pipe_in, 0);
