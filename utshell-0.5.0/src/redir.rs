...
#[no_mangle]
pub fn do_redirections(mut list: *mut REDIRECT, mut flags: libc::c_int) -> libc::c_int {
...
fn expandable_redirection_filename(mut redirect: *mut REDIRECT) -> libc::c_int {
    unsafe {
...
#[no_mangle]
pub fn redirection_expand(mut word: *mut WORD_DESC) -> *mut libc::c_char {
    unsafe {
...
fn add_undo_close_redirect(mut fd: libc::c_int) -> libc::c_int {
    let mut closer: *mut REDIRECT = 0 as *mut REDIRECT;
    let mut sd: REDIRECTEE = REDIRECTEE { dest: 0 };
    sd.dest = fd;
    unsafe {
...
fn add_exec_redirect(mut dummy_redirect: *mut REDIRECT) {
    unsafe {
        (*dummy_redirect).next = exec_redirection_undo_list;
        exec_redirection_undo_list = dummy_redirect;
    }
}
fn stdin_redirection(mut ri: r_instruction, mut redirector: libc::c_int) -> libc::c_int {
    match ri as libc::c_uint {
        r_input_direction
        | r_inputa_direction
        | r_input_output
...
#[no_mangle]
pub fn stdin_redirects(mut redirs: *mut REDIRECT) -> libc::c_int {
    unsafe {
...
fn redir_varassign(mut redir: *mut REDIRECT, mut fd: libc::c_int) -> libc::c_int {
    let mut w: *mut WORD_DESC = 0 as *mut WORD_DESC;
...
fn redir_varvalue(mut redir: *mut REDIRECT) -> libc::c_int {
    unsafe {
        let mut v: *mut SHELL_VAR = 0 as *mut SHELL_VAR;
