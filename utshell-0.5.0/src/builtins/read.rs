...
use crate::bashline::{bashline_reset_event_hook, bashline_set_event_hook, initialize_readline};
use crate::builtins::bashgetopt::{internal_getopt, reset_internal_getopt};
use crate::builtins::common::{
    builtin_bind_variable, builtin_usage, sh_invalidid, sh_invalidnum, sh_ttyerror,
};
...
use crate::subst::{
    dequote_list, dequote_string, get_word_from_string, getifs, list_string,
    strip_trailing_ifs_whitespace, word_list_remove_quoted_nulls,
};
use crate::trap::check_signals;
use crate::unwind_prot::{
    add_unwind_protect, begin_unwind_frame, discard_unwind_frame, remove_unwind_protect,
    run_unwind_frame, unwind_protect_mem,
};
use crate::variables::{bind_variable, get_string_value, stupidly_hack_special_variables};

static mut old_alrm: Option<SigHandler> = None;
static mut reading: libc::c_int = 0;
static mut tty_modified: libc::c_int = 0;
static mut delim: libc::c_char = b'\n' as libc::c_char;
...
pub static mut sigalrm_seen: libc::c_int = 0;
static mut interactive: libc::c_int = 0;
static mut default_buffered_input: libc::c_int = -1;
...
pub fn read_builtin(mut list: *mut WordList) -> i32 {
    ...
    let mut saw_escape: libc::c_int = 0;
    let mut eof: libc::c_int;
    let mut opt: libc::c_int;
    let mut retval: libc::c_int;
    let mut code: libc::c_int;
    let mut print_ps2: libc::c_int;
    let mut nflag: libc::c_int = 0;
    ...
    let mut skip_ctlnul: libc::c_int;

    let mut raw: libc::c_int = 0;
    let mut edit: libc::c_int = 0;
    let mut nchars: libc::c_int = 0;
    let mut silent: libc::c_int = 0;
    let mut have_timeout: libc::c_int = 0;
    let mut ignore_delim: libc::c_int = 0;
    let mut fd: libc::c_int = 0;

    let mut lastsig: libc::c_int = 0;
    let t_errno: libc::c_int;

    let mut _mb_cur_max: libc::c_int;

    let mut tmsec: libc::c_uint = 0;
    let mut tmusec: libc::c_uint = 0;

    let mut ival: libc::c_long = 0;
    let mut uval: libc::c_long = 0;
    ...
    let mut arrayname: *mut libc::c_char = PT_NULL as *mut libc::c_char;

    let mut e: *mut libc::c_char;
    let t: *mut libc::c_char;
    let t1: *mut libc::c_char;
    let mut ps2: *mut libc::c_char;
    let mut tofree: *mut libc::c_char;
    ...
    let vflags: libc::c_int;
    let mut rlbuf: *mut libc::c_char = null_mut();
    let mut itext: *mut libc::c_char = null_mut();

    let mut rlind: libc::c_int = 0;
    ...
    unsafe {
            ...
            match opt_char {
                ...
                't' => {
                    code = uconvert(
                        ...
                        &mut ival,
                        &mut uval,
                        PT_NULL as *mut *mut libc::c_char,
                    );
                    if code == 0 || ival < 0 || uval < 0 {
                        ...
                        have_timeout = 1;
                        tmsec = ival as libc::c_uint;
                        tmusec = uval as libc::c_uint;
                    }
                }
                'N' | 'n' => {
                    if opt_char == 'N' {
        ...
        vflags = if assoc_expand_once != 0 {
            (VA_NOEXPAND | VA_ONEWORD) as libc::c_int
        } else {
            0
        };
        if !list.is_null()
        ...
        if ifs_chars.is_null() {
            ifs_chars = ifs_chars_null.as_ptr() as *mut libc::c_char;
        }
        ...
        loop {
            ...
            skip_ctlesc |= (*e == 1) as libc::c_int;
            skip_ctlnul |= (*e == 117) as libc::c_int;
            e = ((e as usize) + 1) as *mut libc::c_char;
        }

        input_string = malloc(112) as *mut libc::c_char;
        *input_string = b'\0' as libc::c_char;

        'out_assig_vars: loop {
            if nflag == 1 && nchars == 0 {
                let mut gc: libc::c_int = 0;
                retval = libc::read(fd, &mut gc as *mut i32 as *mut c_void, 0) as libc::c_int;
                retval = if retval >= 0 {
                    EXECUTION_SUCCESS
                } else {
                    EXECUTION_FAILURE
            ...
            if (!prompt.is_null() || edit != 0 || silent != 0) && input_is_tty == 0 {
                itext = PT_NULL as *mut libc::c_char;
                edit = 0;
                silent = 0;
            }
            ...
            if tmsec > 0 || tmusec > 0 {
                    ...
                    *input_string.offset(i as isize) = b'\0' as libc::c_char;
                    if i == 0 {
                        ...
                        *t = b'\0' as libc::c_char;
                        ...
                        t = libc::strcpy(
                            malloc((libc::strlen(input_string) + 1) as usize) as *mut libc::c_char,
                            input_string,
                        );
                    }
                    run_unwind_frame(frame_name.as_ptr() as *mut libc::c_char);
                    input_string = t;
                ...
                old_alrm = set_signal_handler(
                    libc::SIGALRM,
                    std::mem::transmute::<fn(_s: libc::c_int), Option<SigHandler>>(sigalrm),
                );
                add_unwind_protect(
                    std::mem::transmute::<fn(), Option<Function>>(reset_alarm),
                    0 as *mut libc::c_void as *mut libc::c_char,
                );

                if edit != 0 {
                    add_unwind_protect(
                        std::mem::transmute::<fn(_cp: *mut libc::c_char), Option<Function>>(
                            reset_attempted_completion_function,
                        ),
                        0 as *mut libc::c_void as *mut libc::c_char,
                    );
                    add_unwind_protect(
                        std::mem::transmute::<fn(), Option<Function>>(bashline_reset_event_hook),
                        0 as *mut libc::c_void as *mut libc::c_char,
                    );
                }
                falarm(tmsec, tmusec);
            ...
            if nchars > 0 || delim != b'\n' as libc::c_char {
                ...
                if edit != 0 {
                    if nchars > 0 {
                        unwind_protect_mem(
                            &mut rl_num_chars_to_read as *mut libc::c_int as *mut libc::c_char,
                            std::mem::size_of_val(&rl_num_chars_to_read) as libc::c_int,
                        );
                        rl_num_chars_to_read = nchars;
                    }

                    if delim != b'\n' as libc::c_char {
                        set_eol_delim(delim as libc::c_int);
                        add_unwind_protect(
                            std::mem::transmute::<fn(_cp: *mut libc::c_char), Option<Function>>(
                                reset_eol_delim,
                            ),
            ...
            if edit != 0 && fd != 0 {
                ...
                unwind_protect_mem(
                    ...
                    std::mem::size_of_val(&rl_instream) as libc::c_int,
                );
                save_instream = rl_instream;
                rl_instream = libc::fdopen(fd, "r".as_ptr() as *const libc::c_char);
            }
            ...
            if nchars > 0 && input_is_tty == 0 && ignore_delim != 0 {
                unbuffered_read = 2;
            } else if nchars > 0 || delim != b'\n' as libc::c_char || input_is_pipe != 0 {
                unbuffered_read = 1;
            }
            ...
            ps2 = PT_NULL as *mut libc::c_char;
            print_ps2 = 0;
            eof = 0;
            retval = 0;
            'get_input_string: loop {
                ...
                if edit != 0 {
                    //没有设置-e edit等于0
                    if !rlbuf.is_null()
                        && *((rlbuf as usize + rlind as usize) as *mut libc::c_char) == 0
                        && delim != 0
                    {
                        libc::free(rlbuf as *mut c_void);
                        rlbuf = PT_NULL as *mut libc::c_char;
                    }
                    if rlbuf.is_null() {
                        reading = 1;
                    ...
                    c = *((rlbuf as usize + rlind as usize) as *mut libc::c_char);
                    rlind += 1;
                } else {
                    if print_ps2 != 0 {
                        if ps2.is_null() {
                            ps2 = get_string_value("PS2".as_ptr() as *const libc::c_char);
                        }
                        eprintln!("{}", CStr::from_ptr(prompt).to_str().unwrap());
                        print_ps2 = 0;
                    }
                ...
                if i + mb_cur_max >= size {
                    size += 128;
                    let t: *mut libc::c_char =
                        realloc(input_string as *mut c_void, size as usize) as *mut libc::c_char;
                    if t != input_string {
                        input_string = t;
                ...
                'out_add_char: loop {
                    ...
                    if c == b'\\' as libc::c_char && raw == 0 {
                        pass_next += 1;
                        if skip_ctlesc == 0 {
                            saw_escape += 1;
                            *((input_string as usize + i as usize) as *mut libc::c_char) =
                                CTLESC as libc::c_char;
                            i += 1;
                        }
                    ...
                    if c == b'\0' as libc::c_char && delim != b'\0' as libc::c_char {
                        continue 'get_input_string;
                    }

                    if (skip_ctlesc == 0 && c == CTLESC as libc::c_char)
                        || (skip_ctlnul == 0 && c == CTLNUL as libc::c_char)
                    {
                        saw_escape += 1;
                        *((input_string as usize + i as usize) as *mut libc::c_char) =
                            CTLESC as libc::c_char;
                ...
                *((input_string as usize + i as usize) as *mut libc::c_char) = c;
                i += 1;
                check_alrm();
                ...
                if mb_cur_max > 1 && is_basic(c) == 0 {
                    *((input_string as usize + i as usize) as *mut libc::c_char) =
                    ...
                    if edit != 0 {
                        let clen = mbrlen(
                            (rlbuf as usize + rlind as usize - 1) as *const libc::c_char,
                            mb_cur_max as size_t,
                        ...
                        if clen > 1 {
                            ...
                            i += clen as libc::c_int - 1;
                            rlind += clen as libc::c_int - 1;
                        }
                    } else if locale_utf8locale == 0 || ((c as u8 & 0x80) != 0) {
                        i += read_mbchar(fd, input_string, i, c as libc::c_int, unbuffered_read);
                    }
                }
                nr += 1;
            ...
            *((input_string as usize + i as usize) as *mut libc::c_char) = b'\0' as libc::c_char;
            check_alrm();
            ...
            discard_unwind_frame(frame_name.as_ptr() as *mut libc::c_char);
        ...
        while !(*list).next.is_null() {
            ...
            if *input_string != 0 {
                ...
                var = bind_read_variable(varname, "".as_ptr() as *mut libc::c_char);
            }
        ...
        tofree = PT_NULL as *mut libc::c_char;
        if *input_string != 0 {
            t1 = input_string;
            t = get_word_from_string(
                std::mem::transmute(&input_string),
...
fn read_mbchar(
    ...
    unsafe {
        ...
        if i > 1 {
            r = 1;
            while r < i as ssize_t {
                *((string as usize + ind as usize + r as usize - 1) as *mut libc::c_char) =
                    mbchar[r as usize];
...
pub fn reset_attempted_completion_function(_cp: *mut libc::c_char) {
    unsafe {
        if rl_attempted_completion_function.is_none() && old_attempted_completion_function.is_some()
        {
...
static mut deftext: *mut libc::c_char = PT_NULL as *mut libc::c_char;

fn set_itext() -> libc::c_int {
    let mut r1 = 0;
    let mut r2 = 0;
    ...
    return (r1 != 0 || r2 != 0) as libc::c_int;
}

fn edit_line(p: *mut libc::c_char, itext: *mut libc::c_char) -> *mut libc::c_char {
    let mut len: i32;
    unsafe {
        if bash_readline_initialized == 0 {
            initialize_readline();
        ...
        ret = realloc(ret as *mut c_void, (len + 2) as usize) as *mut libc::c_char;
        *ret.offset(len as isize) = delim;
        len += 1;
        *ret.offset(len as isize) = b'\0' as libc::c_char;
        return ret;
    }
}

fn sigalrm(_s: libc::c_int) {
    unsafe {
        sigalrm_seen = 1;
    }
}
...
#[no_mangle]
pub fn read_tty_modified() -> libc::c_int {
    unsafe {
        return tty_modified;
    }
}
...
fn set_eol_delim(c: libc::c_int) {
    ...
    unsafe {
        ...
        old_delim_ctype = (*cmap.offset(c as isize)).Type as libc::c_int;
        old_delim_func = (*cmap.offset(c as isize)).function;
...
fn reset_eol_delim(_cp: *mut libc::c_char) {
    unsafe {
        ...
        (*((cmap as usize + ret_pos) as Keymap)).Type = old_newline_ctype as libc::c_char;
        (*((cmap as usize + ret_pos) as Keymap)).function = std::mem::transmute(old_newline_func);

        (*((cmap as usize + delim_pos) as Keymap)).Type = old_delim_ctype as libc::c_char;
