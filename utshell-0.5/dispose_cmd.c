...
// extern r_dispose_command(COMMAND *command);
void
    dispose_command(command)
        COMMAND *command;
{
  // r_dispose_command(command);
  if (command == 0)
    return;
...
// extern r_dispose_cond_node(COND_COM *cond);
void
    dispose_cond_node(cond)
        COND_COM *cond;
{
  // r_dispose_cond_node(cond);
  if (cond)
  {
    if (cond->left)
...
#endif /* COND_COMMAND */
// extern r_dispose_function_def_contents(FUNCTION_DEF *c);
void
    dispose_function_def_contents(c)
        FUNCTION_DEF *c;
{
  // r_dispose_function_def_contents(c);
//   // dispose_word(c->name);
//   // dispose_command(c->command);
//   // FREE(c->source_file);
}
// extern r_dispose_function_def(FUNCTION_DEF *c);
void
    dispose_function_def(c)
        FUNCTION_DEF *c;
{
  // r_dispose_function_def(c);
  dispose_function_def_contents(c);
  free(c);
}

/* How to free a WORD_DESC. */
// extern r_dispose_word(WORD_DESC *w);
void
    dispose_word(w)
        WORD_DESC *w;
{
...
// extern r_dispose_word_desc(WORD_DESC *w);
void
    dispose_word_desc(w)
        WORD_DESC *w;
{
  // r_dispose_word_desc(w);
  w->word = 0;
  // ocache_free (wdcache, WORD_DESC, w);
  free(w);
}
...
// extern r_dispose_words(WORD_LIST *list);
void
    dispose_words(list)
        WORD_LIST *list;
{
  // r_dispose_words(list);
  WORD_LIST *t;
...
void
    dispose_word_array(array) char **array;
{
  register int count;
...
// extern r_dispose_redirects( REDIRECT *list);
void
    dispose_redirects(list)
        REDIRECT *list;
{
  // r_dispose_redirects(list);
  register REDIRECT *t;
