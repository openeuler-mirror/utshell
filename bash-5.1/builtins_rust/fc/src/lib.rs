...
extern "C" {
    ...
    static mut lcurrent:*mut WORD_LIST;
...
#[no_mangle]
pub extern "C" fn r_fc_number (list:* mut WORD_LIST)->i32
{
...
unsafe fn STREQN ( a:* const c_char, b:* const c_char, n:i32)->bool {
  if n==0 {
    ...
    return *((a as usize +4) as * const c_char) ==*((b as usize +4) as * const c_char) && libc::strncmp(a, b, n as libc::size_t) == 0
  }
}

#[no_mangle]
pub extern "C" fn r_fc_builtin (list:* mut WORD_LIST)->i32
{
  let mut i:i32;
  ...
  loptend = lcurrent;
  let mut ret:bool=r_fc_number (loptend) ==0;
  ...
  if execute != 0 {
      ...
      sep= libc::strchr((*(*llist).word).word, char::from('=') as libc::c_int);
      ...
      while ret	{
        sep= (sep as usize + 4) as * mut c_char;
	      *sep = char::from('\0') as c_char ;
	      ...
	      (*rl).pat = savestring ((*(*llist).word).word);
      ...
      if llist != std::ptr::null_mut() {
        command=r_fc_gethist((*(*llist).word).word, hlist, 0);
      } else {
  ...
  i=0;
  while  *((hlist as usize +(4*i) as usize) as  * mut * mut HIST_ENTRY) != std::ptr::null_mut()  {    
    i+=1;
  }
  ...
  real_last = i;
  while *((hlist as usize +(4*real_last) as usize ) as  * mut * mut HIST_ENTRY) == std::ptr::null_mut() && real_last > 0
  {
    real_last-=1;
  }
  
  if i == last_hist && *((hlist as usize + (4*last_hist) as usize) as  * mut * mut HIST_ENTRY) == std::ptr::null_mut() {
  ...
  if llist != std::ptr::null_mut()  {
      histbeg = r_fc_gethnum ((*(*llist).word).word, hlist, listing| HN_FIRST!());
      llist = (*llist).next;
  ...
  if listing == 0 && hist_last_line_added !=0 {
      ...
      if histbeg == histend && histend == last_hist && *((hlist as usize + (4*last_hist) as usize) as  * mut * mut HIST_ENTRY)  == std::ptr::null_mut() {
        histend-=1;
        last_hist = histend;
        histbeg = histend;
      }

      if *((hlist as usize + (4*last_hist) as usize) as  * mut * mut HIST_ENTRY)  == std::ptr::null_mut() {
        last_hist-=1;
      }
  ...
  while ret {
      ...
      if listing !=0 {
        if posixly_correct !=0 {
          ...
          let mut ch:char;
          if (**((hlist as usize + i as usize) as *mut*mut HIST_ENTRY)).data != std::ptr::null_mut() {
            ch='*';
          } else {
            ch=' ';
          }
      ...
      if stream != std::ptr::null_mut() {
        libc::fprintf (stream, CString::new ("%s\n").unwrap().as_ptr(), (**((hlist as usize + (i*4) as usize) as *mut*mut HIST_ENTRY)).line);
      }else {   
        printToStdout((**((hlist as usize + (i*4) as usize) as *mut*mut HIST_ENTRY)).line);
      }
...
pub extern "C" fn r_fc_gethist (command:* mut c_char, hlist:* mut * mut HIST_ENTRY, mode:i32)->* mut c_char
{
  ...
  unsafe {
    if i >= 0 {
      return savestring ((*(*((hlist as usize + (4*i) as usize) as * mut * mut HIST_ENTRY))).line );
    }  else {
      return std::ptr::null_mut();
    }
  }
...
pub extern "C" fn r_fc_gethnum (command:* mut c_char, hlist:* mut * mut HIST_ENTRY, mode:i32)->i32
{
  ...
  if s != std::ptr::null_mut() && DIGIT( *s ) {
    n = std::ffi::CStr::from_ptr(s).to_str().unwrap().parse::<i32>().unwrap();
    n *= sign;
  ...
  j = i;
  while j >= 0 {
    if STREQN (command, (*(*((hlist as usize + (4*j) as usize ) as  * mut * mut HIST_ENTRY))).line, clen) {
      return j;
    }
    j-=1;
  }
